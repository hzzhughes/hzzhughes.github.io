...
$$x^{r+1}=x^r-\alpha^r\nabla f(x^r)=\argmin_x\lang x-x',\nabla f(x')\rang+\frac{1}{2\alpha^r}||x-x^r||^2$$
...
$$x^{r+1}=\argmin_xf_0(x)+\lang x-x^r,\nabla f_1(x^r)\rang+\frac{1}{\alpha^r}B_\eta(x,x^r)$$
